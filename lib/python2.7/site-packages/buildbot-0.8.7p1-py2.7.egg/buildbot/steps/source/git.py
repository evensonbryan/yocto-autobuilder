...
class Git(Source):
    ...
    def __init__(self, repourl=None, branch='HEAD', mode='incremental', srcdir='sources',
                 builddir='build', method=None, submodules=False, shallow=False, progress=False,
                 retryFetch=False, clobberOnFailure=False, getDescription=False, layername=None,
                 **kwargs):
        """
        @type  repourl: string
        ...
        if self.mode == 'full':
            assert self.method in ['clean', 'fresh', 'clobber', 'copy', 'movecopy', None]
    ...
    def startVC(self, branch, revision, patch):
        ...
        log.msg("Method" + str(self.method))
        d = self.checkGit()
    ...
    def movecopy(self):
        # This relies on an external method of cleaning up the todir. cron jobs
        # or some such method.
        import time
        import random
        ...
        cmd = buildstep.RemoteCommand('mv', {'fromdir': self.builddir,
                                                                    'todir': self.storedir + '/' + self.getProperty("DESTDIR") + '/' + 
                                                                                str(int(time.time())) + '-'  + 
    ...
    def copy(self):
        cmd = buildstep.RemoteCommand('rmdir', {'dir': self.builddir,
                                                'logEnviron': self.logEnviron,})
        cmd.useLog(self.stdio_log, False)
        d = self.runCommand(cmd)
    ...
    def _fetch(self, _):
        ...
        d = self._dovccmd(command)
        def checkout(_):
            if self.revision and self.revision is not 'HEAD':
                rev = self.revision
            else:
                rev = 'FETCH_HEAD'
