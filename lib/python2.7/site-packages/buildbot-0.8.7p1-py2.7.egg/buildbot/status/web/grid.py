                ...
                if len(build.getSourceStamps()) == 1:
                    ...
                    for i in range(len(stamps)):
                        if key == self.getSourceStampKey(stamps[i]) and builds[i] is None:
                            builds[i] = build
                for name, value, source in build.getProperties().asList():
                    if "branch_" in name and "trigger" not in name:
                        cxt.update({name.replace("-","_"): value})
                    if "repourl_" in name and "trigger" not in name:
                        cxt.update({name.replace("-","_"): value})
                    if "custom_release_name_" in name:
                        cxt.update({name.replace("-","_"): value})
