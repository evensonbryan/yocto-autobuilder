...
class CreateAutoConf(ShellCommand):
    ...
    def start(self):
        ...
        if self.atext is not "":
            ...
            try:
                buildappsrcrev=self.getProperty("custom_buildappsrcrev_build-appliance")
                log.msg(buildappsrcrev)
                log.msg(self.machine)
            except:
                pass
            if self.buildapp is True:
                ...
                if buildappsrcrev != "DEFAULT":
                    fout = fout + 'SRCREV_pn-build-appliance-image = "' + buildappsrcrev + '"\n'
            else:
