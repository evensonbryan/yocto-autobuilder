...
class CreateAutoConf(ShellCommand):
    """
    Build step to create the auto.conf file.
    """
    haltOnFailure = False
    flunkOnFailure = True
    name = "CreateAutoConf"
    def __init__(self, factory, argdict=None, **kwargs):
        """
        argdict contents:
            "atextprepend": string -- prepends text to auto.conf
            "atext": string -- overrides all other config text (after prepend) in auto.conf
            "distro": string -- sets the DISTRO setting
            "packages": string -- space separated list of packages
            "buildapp": bool -- buildapp setting...???
            "emgd": bool -- whitelists emgd driver licenses if True
            "pvr": bool -- whitelists pvr driver licenses if True
            "multilib": string or bool -- includes multilib:lib32 if True or "lib32", or multilib:lib64 if "lib64"
            "nosstate": bool -- sets the SSTATE_DIR if false
            "machine": string -- used for the MACHINE setting
            "swabber": bool -- image-swab setting...????
            "buildhistory": bool -- if BUILD_HISTORY_COLLECT and on poky master branch, include build history items
            "initmgr": string -- init manager setting
            "adtdev": bool -- adt repo stuff...???
            "atextappend": string -- appends text at end of auto.conf (even after atext override)
        """
    ...
    def start(self):
        ...
        if self.atext:
            ...
            if self.buildhistory == True and os.environ.get("BUILD_HISTORY_COLLECT") == "True" and self.getProperty("branch_poky") == "master":
