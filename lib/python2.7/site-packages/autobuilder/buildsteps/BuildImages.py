...
class BuildImages(ShellCommand):
    ...
    def start(self):
        ...
        if self.layerversion_yoctobsp is not None and int(self.layerversion_yoctobsp) < 2 and machine == "genericx86-64":
    ...
    def getText(self, cmd, results):
        ...
        if self.layerversion_yoctobsp is not None and int(self.layerversion_yoctobsp) < 2 and machine == "genericx86-64":
            text.append("genericx86-64 does not exist in this branch. Skipping")
        else:
            text.append("Building " + str(self.images))
        return text
