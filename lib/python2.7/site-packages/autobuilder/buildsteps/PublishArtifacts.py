        ...
        if str(os.environ.get('PUBLISH_BUILDS')) == "True":
            for artifact in self.artifacts:
                ...
                if artifact == "adt-installer":
                    ...
                    command=command+"cp -R --no-dereference --preserve=links " + \
                                    os.path.join(self.basedir, "tmp/deploy/sdk/poky-*glibc-i686-core-image-*sdk*.sh ") + \
                                    os.path.join(DEST, X86TC_PUBLISH_DIR) + ";"
                    command=command+"mkdir -p " + os.path.join(DEST, X8664TC_PUBLISH_DIR) + ";"
                    command=command+"cp -R --no-dereference --preserve=links " + \
                                    os.path.join(self.basedir, "tmp/deploy/sdk/poky-*glibc-x86_64-core-image-*sdk*.sh ") + \
                    ...
                    command=command+" if [ -d " + deploy_image_dir + " ] ; then cp -R --no-dereference --preserve=links " + \
                                     deploy_image_dir + \
                                    "/*.md5sum " + DEST + "/" + MACHINE_PUBLISH_DIR + "/" + artifact_name + " ; fi ;"
