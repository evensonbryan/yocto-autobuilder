'''
Created on Dec 26, 2012

__author__ = "Elizabeth 'pidge' Flanagan"
__copyright__ = "Copyright 2013, Intel Corp."
__credits__ = ["Elizabeth Flanagan"]
__license__ = "GPL"
__version__ = "2.0"
__maintainer__ = "Elizabeth Flanagan"
__email__ = "elizabeth.flanagan@intel.com"
'''

from buildbot.steps.shell import ShellCommand


class CreateBBLayersConf(ShellCommand):
    haltOnFailure = True 
    flunkOnFailure = True 
    name = "Create BBLayers Configuration" 
    def __init__(self, factory, layers, argdict=None, **kwargs):
        self.factory = factory
        self.layers=layers
        for k, v in argdict.iteritems():
            setattr(self, k, v)
            print "setting " + str(k) + " " + str(v)
        self.description = "Create BBLayers Configuration"
        ShellCommand.__init__(self, **kwargs)

    def start(self):
        import os
        fout = ""
        buildername=self.getProperty("buildername")
        branch=self.getProperty("branch")
        workdir=self.getProperty("workdir")
        for line in open(workdir + "/build/build/conf/bblayers.conf"):
            if "LCONF_VERSION" in line:
                LCONF_VERSION=str(line.split("=")[-1]).replace('"', '').strip()
                break
        print LCONF_VERSION
        #check to see if we have a prepend
        if self.bbtextprepend is not "":
            fout = fout + self.bbtextprepend
        #check to see if we have text override
        if self.bbtext is not "":
            fout = fout+self.bbtext
        
        fout = fout + 'LCONF_VERSION = "' + LCONF_VERSION +'" \n' 
        fout = fout + 'BBPATH = "${TOPDIR}" \n'
        fout = fout + 'BBFILES ?="" \n'
        fout = fout + 'BBLAYERS += " \ \n'
        if self.buildprovider == "yocto":
            fout = fout + workdir + "/build/meta \ \n"
            fout = fout + workdir + "/build/meta-yocto \ \n"
            if int(LCONF_VERSION) > 5:
                fout = fout + workdir + '/build/meta-yocto-bsp \ \n'
                if self.x32abi == True:
                    fout = fout + workdir + '/build/meta-x32 \ \n'
        elif self.buildprovider == "oe":
            fout = fout + workdir + "/build/meta \ \n"
        if self.bsplayer == True and self.bspprovider == "intel":
            if branch != "edison":
                fout = fout + workdir + '/build/yocto/meta-intel' + ' \ \n'
            fout = fout + workdir + '/build/yocto/meta-intel/meta-' + buildername.replace("-noemgd", "") + ' \ \n'
            fout = fout + workdir + '/build/yocto/meta-intel/meta-tlk \ \n'
        elif self.bsplayer == True and self.bspprovider == "fsl-ppc":
            fout = fout + workdir + '/build/yocto/meta-fsl-ppc \ \n'
        elif self.bsplayer == True and self.bspprovider == "fsl-arm":
            fout = fout + workdir + '/build/yocto/meta-fsl-arm \ \n'
        for layer in dict(self.layers):
            if "meta-qt3" in layer:
                fout = fout + workdir + '/build/meta-qt3 \ \n'
                break
        fout = fout + '" \n'
        self.command = "rm -rf ./build/conf/bblayers.conf; echo '" +  fout + "'>> ./build/conf/bblayers.conf"
        ShellCommand.start(self)

    def describe(self, done=False):
        description = ShellCommand.describe(self,done)
        return description
