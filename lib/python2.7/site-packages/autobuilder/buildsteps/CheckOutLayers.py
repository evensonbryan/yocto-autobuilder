...
'''
# Inheriting the git object was breaking things and I needed this out of the 
# way. This is a very temporary measure until I can get the inheritence correct.
# Honestly, we should be using codebases anyway, but until I can look at 
# implementation/patches to buildbot, this works.
...
class CheckOutLayers (Source):
    ...
    def __init__(self, factory, scheduler=None, layername=None, mode='full', 
                ...
                progress=True, retryFetch=False, clobberOnFailure=False, 
    ...
    def startVC(self, branch, revision, patch):
        if not self.getProperty('branch_trigger_'+self.scheduler+"_"+self.layername):
            self.branch = self.getProperty('branch_'+self.layername)
        else:
            self.branch = self.getProperty('branch_trigger_'+self.scheduler+"_"+self.layername)
        if not self.getProperty('repo_trigger_'+self.scheduler+"_"+self.layername):
            self.repourl = self.getProperty('repo_'+self.layername)
        else:
            self.repourl = self.getProperty('repo_trigger_'+self.scheduler+"_"+self.layername)
        self.setProperty('repourl_' + self.layername, self.repourl)
        self.setProperty('branch_' + self.layername, self.branch)
        log.msg("Branch :" + str(branch))
    ...
    def parseGotRevision(self, _=None):
        ...
        if self.layername is "poky":
            self.updateSourceProperty('got_revision', revision)
        else:
            self.setProperty('got_revision_'+self.layername, revision) 
        defer.returnValue(0)
