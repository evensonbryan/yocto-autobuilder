    ...
    def start(self):
        ...
        if self.layerversion_core < "4":
            ...
            if str(os.environ.get('ERROR_REPORT_COLLECT')) == "True":
                ...
                self.command = "if [ -d " + self.errordir +"/$x" + " ]; then for x in `ls |grep error_report_ " + self.errordir +"`; do send-error-report " + self.errordir +"/$x" + self.errorreportserver + "; done; fi" 
