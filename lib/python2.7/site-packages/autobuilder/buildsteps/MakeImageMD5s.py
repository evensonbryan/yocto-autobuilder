...
class MakeImageMD5s(ShellCommand):
    ...
    def start(self):
        ...
        if str(os.environ.get('PUBLISH_BUILDS')) == "True":
            command = command + "cd build/tmp/deploy/images; find . -type f "
            command = command + "-exec md5sum {} \; >> " + buildername
            command = command + ".md5sum; mkdir -p " + DEST + "/build_md5sums;" 
            command = command + "rsync -av " + buildername + ".md5sum " 
            command = command + DEST + "/build_md5sums"
        else:
